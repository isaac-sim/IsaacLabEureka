...
from isaaclab_eureka.utils import MuteOutput, get_freest_gpu, turn_cfg_to_string

DIRECT_TEMPLATE_REWARD_STRING = """
...
def eureka_worker_main(
    ...
    """Top-level worker entrypoint. Must be defined at module scope for Windows compatibility."""
    worker = _EurekaWorker(
        idx,
        rewards_queue,
        observations_queue,
        results_queue,
...
class _EurekaWorker:
    def __init__(
        ...
        self._observations_queue = observations_queue
    ...
    def _create_environment(self):
        ...
        import Reach_Shadow_UR10.tasks  # noqa: F401
    ...
    def _prepare_direct_eureka_environment(self, get_rewards_method_as_string: str):
        ...
        if not hasattr(env, "_get_rewards_eureka"):
            ...
            template_reward_string_with_module = DIRECT_TEMPLATE_REWARD_STRING.format(module_name=env.__module__)
    ...
    def _prepare_manager_eureka_environment(self, get_rewards_method_as_string: str):
        """Prepare the environment for training with the Eureka-generated reward function.
    ...
    def run_loop(self):
        while not self._termination_event.is_set():
            if not hasattr(self, "_env"):
                ...
                # Only send back the observations string once
                if self._idx == 0:
                    if hasattr(self._env.unwrapped, "_get_observations"):
                        self._observation_string = inspect.getsource(self._env.unwrapped._get_observations)
                    elif hasattr(self._env.unwrapped, "observation_manager"):
                        self._observation_string = turn_cfg_to_string({"observation_manager": self._env.unwrapped.observation_manager.cfg,
                                                                        "reward_manager": self._env.unwrapped.reward_manager.cfg})
                        # print(self._observation_string)
                    else:
                        raise RuntimeError(
                            "The environment does not have a _get_observations method or an observation manager."
                        )
            ...
            if isinstance(reward_func_string, str) and reward_func_string.startswith("def _get_rewards_eureka(self)"):
                ...
                result = {"success": False, "exception": "Reward function format invalid."}
...
class EurekaTaskManager:
    ...
    def __init__(
        ...
        self._rewards_queues = [ctx.Queue() for _ in range(self._num_processes)]
        self._observations_queue = ctx.Queue()
        self._results_queue = ctx.Queue()
        self.termination_event = ctx.Event()

        for idx in range(self._num_processes):
            p = ctx.Process(
                target=eureka_worker_main,
                args=(
                    idx,
